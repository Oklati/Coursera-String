...
void AddToTree(tree* node, int pos, const int& suffix){
	if(pos == text.size()){
		node->suf = suffix;
		return ;
	}
	...
	for(int i=pos; i< text.size(); i++){
		if(commonLen == node->edge[c].S){
			...
			return AddToTree(node->child[c], pos+ commonLen, suffix);
		}
	...
	return AddToTree(newnode, pos+commonLen, suffix);
}
